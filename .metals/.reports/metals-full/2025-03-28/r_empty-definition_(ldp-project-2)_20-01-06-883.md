error id: project_2/main.BJKSTSketch#z().
file://<WORKSPACE>/src/main/scala/project_2/main.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol project_2/main.BJKSTSketch#z().
...
/* A constructor that requies intialize the bucket and the z value. The bucket size is the bucket size of the sketch. */
    ...
    def +(that: BJKSTSketch): BJKSTSketch = {    /* Merging two sketches */
      ...
      val new_z = trimmed.map(_._2).max
    ...
    def add_string(s: String, z_of_s: Int): BJKSTSketch = {
      ...
      if (new_bucket.size <= BJKST_bucket_size) {
        val new_z = math.min(z, z_of_s)
        new BJKSTSketch(new_bucket, new_z, BJKST_bucket_size)
      } else {
        // Trim to BJKST_bucket_size elements with lowest z
        val trimmed = new_bucket.toSeq.sortBy(_._2).take(BJKST_bucket_size).toSet
  ...
  def BJKST(x: RDD[String], width: Int, trials: Int): Double = {
    ...
    val emptySketches = Seq.fill(trials)(new BJKSTSketch(Set.empty[(String, Int)], Int.MaxValue, width))

    val mergedSketches = x.aggregate(emptySketches)(
      (acc, s) => {
        acc.indices.map { i =>
          val z = h(i).zeroes(h(i).hash(s))
          if (z <= acc(i).z) acc(i).add_string(s, z) else acc(i)
        }
      },
      (acc1, acc2) => {
        acc1.indices.map(i => acc1(i) + acc2(i))
      }
    )

    val estimates = mergedSketches.map { sk =>
      if (sk.z == Int.MaxValue || sk.bucket.isEmpty) {
        0.0
      } else {
        sk.bucket.size * math.pow(2, sk.z@@)
      }
    }

    val nonzeroEstimates = estimates.filter(_ > 0)
    val median = if (nonzeroEstimates.nonEmpty) {
      val sorted = nonzeroEstimates.sorted
      sorted(nonzeroEstimates.size / 2)
    } else {
      0.0
    }
