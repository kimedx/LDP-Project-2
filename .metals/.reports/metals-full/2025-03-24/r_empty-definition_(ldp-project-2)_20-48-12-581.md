error id: file://<WORKSPACE>/src/main/scala/project_2/main.scala:116
file://<WORKSPACE>/src/main/scala/project_2/main.scala
empty definition using pc, found symbol in pc: 
semanticdb not found
|empty definition using fallback
non-local guesses:
	 -org/apache/spark/sql/functions/mergedSketches.
	 -org/apache/spark/sql/functions/mergedSketches#
	 -org/apache/spark/sql/functions/mergedSketches().
	 -org/apache/spark/rdd/mergedSketches.
	 -org/apache/spark/rdd/mergedSketches#
	 -org/apache/spark/rdd/mergedSketches().
	 -mergedSketches.
	 -mergedSketches#
	 -mergedSketches().
	 -scala/Predef.mergedSketches.
	 -scala/Predef.mergedSketches#
	 -scala/Predef.mergedSketches().
...
/* A constructor that requies intialize the bucket and the z value. The bucket size is the bucket size of the sketch. */
  ...
  def BJKST(x: RDD[String], width: Int, trials: Int) : Double = {
    val h = Seq.fill(trials)(new hash_function(1L << 20))  // Large enough buckets
    val sketches = Seq.fill(trials)(new BJKSTSketch(Set(), Int.MaxValue, width))
    ...
    val estimates = mergedSketches.map(sk => math.pow(2, sk.z))
  ...
  def Tug_of_War(x: RDD[String], width: Int, depth:Int) : Long = {
    val hashes = Seq.fill(depth)(new four_universal_Radamacher_hash_function)

    val estimates = hashes.map { h =>
      val buckets = x
        .map(s => (h.hash(s), 1L))  // ±1 values
        .reduceByKey(_ + _)
        .map { case (_, sum) => sum * sum }
        .sum()
  
      buckets
    }
  
    (estimates.sum / depth).toLong  // Average over depth
  }
